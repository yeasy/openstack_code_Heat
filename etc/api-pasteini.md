...
```
所以实际上加载的还是 heat.api.openstack.v1.API。
而 heat.api.openstack.v1:API 中定义了对各种资源（包括 stack、resource、event、action、info、software config、software deployment 等）进行操作的 URL 所对应的处理方法。经过简单处理，后会发出对应的 rpc 消息给 heat-engine。
